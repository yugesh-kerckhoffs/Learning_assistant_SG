...
import { shapesData } from '@/data/gameData';
...
const ShapesGamePage: React.FC = () => {
  ...
  const initLevel = useCallback((lv: number) => {
    const data = shapesData[Math.min(lv, 10)];
    if (!data) return;
  ...
  const saveToDb = async (lv: number, completed: boolean) => {
    ...
    try { await externalSupabase.from('colors_shapes_sessions').insert({ user_id: currentUser.id, game_type: 'shapes', level_reached: lv, completed }); } catch {}
  };
    ...
    <>
      {showTrophy && (
        <div style={trophyOverlayStyle}>
          ...
          <p style={{ color: 'white', fontSize: '1.3em', marginTop: '10px' }}>You completed all 10 levels of Shapes!</p>
        </div>
      )}
      ...
      <div className="colors-shapes-game-container">
        <h2 className="colors-shapes-game-title">⬛ Shapes Matching - Level {level}</h2>
